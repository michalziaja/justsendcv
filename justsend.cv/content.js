...
if (window.location.href.startsWith('http://justsend.cv')) {
...
async function checkButtonState(saveButton) {
    ...
    chrome.storage.local.get(['supabase_access_token'], async (result) => {
        if (result.supabase_access_token) {
            try {
                const response = await fetch('http://justsend.cv/api/user/me', {
                    headers: {
...
function createSaveButton() {
    ...
    saveButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const userResponse = await fetch('http://justsend.cv/api/user/me', {
                headers: {
            ...
            const response = await fetch('http://justsend.cv/api/jobs', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                chrome.storage.local.set({ 
                    ...
                    chrome.runtime.sendMessage({
                        ...
                        targetUrl: 'http://justsend.cv/saved'
                    }).catch(error => {
